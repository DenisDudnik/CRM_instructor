...
class User(AbstractUser):
    ...
    role = models.CharField(
        verbose_name='роль', max_length=1, choices=ROLES
    )
    lessons = models.ManyToManyField(Lesson, related_name='users')
    manager = models.ForeignKey('self', on_delete=models.SET_NULL,
                                related_name='users', null=True)
    salary = models.FloatField(verbose_name='ставка', null=True)
